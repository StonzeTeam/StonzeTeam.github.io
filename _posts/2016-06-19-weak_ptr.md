...
자신이 가리키는 대상이 이미 파괴되었을 수도 있따는 문제를 극복할 수 있어야 한다. 스마트 포인터는 가리키는 객체에 대하여 해당 객체가 유효한지 그렇지 않은지에 대하여 알 수 있어야한다.
...
허나 그 객체의 참조 횟수에는 영향을 주지 않는다. 이전 스터디 자료에서 보았듯이 shared_ptr은 control block이라는 자료구조를 가지고 있다. 이 자료구조에는 커스텀 할당자, 커스텀 삭제자와 더불어
...
같은 문제상황이 발생할 수 있다. 따라서 이러한 문제점을 해결하기 위해서는 "만료검사 + 객체 참조" 에 대한  연산을 하나의 원자적 연산으로 처리해야한다.
weak_ptr로부터 shared_ptr을 생성하면된다. 
...
따라서 캐시에 저장할 포인터는 자신이 대상을 잃었음을 감지할 수 있는 weak_ptr이 되어야 한다. 또한  weak_ptr을 사용하려면 shared_ptr이 있어야한다.
따라서 팩토리 함수의 반환형형식도 shared_ptr이 되어야한다.
